            ...
            <Container>
                <Section column>
                    <TabsContainer>
                        <Button
                            to="b64"
                            data-tab={TABS.IMAGE_TO_B64}
                            onClick={handleTabChangeClick}
                            disabled={activeTab === TABS.B64_TO_IMAGE}
                        >
                            Image to base64
                        </Button>
                        <Button
                            to="b64"
                            data-tab={TABS.B64_TO_IMAGE}
                            onClick={handleTabChangeClick}
                            disabled={activeTab === TABS.IMAGE_TO_B64}
                        >
                            Base64 to image
                        </Button>
                    </TabsContainer>

                    {activeTab === TABS.IMAGE_TO_B64 && <ImageToBase64 />}

                    {activeTab === TABS.B64_TO_IMAGE && <Base64ToImage />}
                </Section>
            </Container>
        </>
    )
